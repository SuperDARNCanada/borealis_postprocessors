...
if __name__ == "__main__":
    ...
    # Update the file
    fixed_file = file_updater(args.filename, args.fixed_data_dir)

    # Convert to standard bfiq.hdf5.array non zipped and save
    reader = pydarnio.BorealisRead(fixed_file, 'antennas_iq', 'site')
    bfiq_file = fixed_file.split('.')
    bfiq_file = 'bfiq.hdf5.array'
    bfiq_file = '.'.join(bfiq_file[0:6])
    pydarnio.BorealisWrite(bfiq_file, reader.records, 'bfiq', 'array')

    # Process bfiq to rawacf
    rawacf_file = fixed_file.split('.')
    rawacf_file = 'rawacf.hdf5.array'
    rawacf_file = '.'.join(rawacf_file[0:6])
    bfiq2rawacf(bfiq_file, rawacf_file, 'array', 'array')
