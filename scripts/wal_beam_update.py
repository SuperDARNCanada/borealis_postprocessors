...
def hdf5_fixer(path: str):
    ...
    if structure == "dmap":
        raise ValueError(f"Unknown structure for file {os.path.basename(path)}")

    with h5py.File(path, "r+") as f:
        if structure == "array":
            new_beam_dirs = np.round(
                np.rad2deg(
                    np.arcsin(
                        old_spacing
...
if __name__ == "__main__":
    ...
    for i, infile in enumerate(files):
        print(f"{i:>5d}/{num_files}  {infile}", end="")
        ...
        try:
                ...
                if args.for_real:
                    if args.reprocess:
                        processor = UpdateBeamDirs(
                            infile,
                            borealis_to_borealis_rename(infile, "rawacf", "site"),
            ...
            if "rawacf" in infile:
                ...
                raise ValueError(
                    f"Unknown file type for file: {os.path.basename(infile)}"
                )
        except:
